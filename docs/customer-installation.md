...
# GitBook Worker Anwenderanleitung v2.9.0

Diese Anleitung ist der zentrale Einstieg fuer Anwenderinnen und Anwender von
GitBook Worker v2.9.0. Sie beschreibt Installation, Projektstruktur,
...
fuer v2.9.0 freigegeben. Die Checkliste am Ende bleibt als Liefer- und
...
- Python 3.10 oder neuer; fuer v2.9.0 ist Python 3.11/3.12 empfohlen.
- Pandoc.
...
```powershell
python -m pip install --force-reinstall dist\gitbook_worker-2.9.0-py3-none-any.whl
```
...
- Fuer pip-/sdist-basierte Docker-CI bitte mindestens `2.9.0` verwenden. `2.4.1`
...
- Der PDF-Textscan findet positive CJK-, Devanagari- und Ethiopic-Signale.
- `font_cli.py stats --fail-on-targets` prueft die fertigen ERDA-TTFs direkt:
  `erda-ccby-cjk.ttf` muss mindestens 3000 Han- und 3000 Hangul-Zielglyphen
  sowie vollstaendige Kana-Ziele melden; Indic/Ethiopic muessen ihre
...
1. Release Notes lesen: [v2.9.0 Release Notes](releases/v2.9.0.md).
...
## 19. Release-Verifikation v2.9.0

Der v2.9.0-Release wird lokal mit diesen Signalen geprueft:

- Focused Checkbox-/Emoji-Regressionen: `2 passed, 6 deselected`.
- Publisher-Default-Filter-Regression: `1 passed, 33 deselected`.
- Non-slow Test-Suite: `585 passed, 11 skipped, 10 deselected, 4 warnings`.
- `git diff --check`, `git diff --cached --check`, `py_compile` und
  `black --check` fuer die beruehrten Python-Dateien: bestanden.
- Sauberer Wheel- und sdist-Build fuer `gitbook_worker-2.9.0`: bestanden;
  `gitbook_worker-2.9.0.tar.gz` und
  `gitbook_worker-2.9.0-py3-none-any.whl` wurden erzeugt.
- Wheel-Smoke in frischer virtueller Umgebung ausserhalb des Repositorys:
  bestanden; Import meldete `version: 2.9.0` und die
