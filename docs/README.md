---
version: 1.0.14
date: 2026-05-12
history:
	- "1.0.14: 2026-05-12 - Anwenderdokumentationsindex fuer v2.9.2 Layoutpakete aktualisiert"
	- "1.0.13: 2026-05-11 - Anwenderdokumentationsindex fuer v2.9.1 Abnahmefix aktualisiert"
	- "1.0.12: 2026-05-10 - Anwenderdokumentationsindex fuer v2.9.0 Qualitaetskompass aktualisiert"
	- "1.0.11: 2026-05-10 - Editorial-Quality-Liefernachweise fuer v2.9.0 verlinkt"
	- "1.0.10: 2026-05-10 - Lieferreife-Review fuer v2.9.0 Qualitaetskompass verlinkt"
	- "1.0.9: 2026-05-10 - Redakteursreport fuer v2.9.0 Qualitaetskompass verlinkt"
	- "1.0.8: 2026-05-09 - Editorial Quality Tools fuer v2.9.0 Qualitaetskompass verlinkt"
	- "1.0.7: 2026-05-09 - Anwenderdokumentationsindex fuer v2.8.0 Tabellenprofi aktualisiert"
	- "1.0.6: 2026-05-08 - Anwenderdokumentationsindex fuer v2.7.0 Wide-Table-Paper-Selection aktualisiert"
	- "1.0.5: 2026-05-08 - Anwenderdokumentationsindex fuer v2.6.1 URL-Code-Fence-Hotfix aktualisiert"
	- "1.0.4: 2026-05-07 - Anwenderdokumentationsindex fuer v2.6.0 Code-Fence-Wrapping aktualisiert"
	- "1.0.3: 2026-05-07 - Anwenderdokumentationsindex fuer v2.5.0 Font-Coverage aktualisiert"
	- "1.0.2: 2026-05-07 - Anwenderdokumentationsindex fuer v2.4.2 aktualisiert"
	- "1.0.1: 2026-05-06 - Anwenderdokumentationsindex fuer v2.4.2 aktualisiert"
	- "1.0.0: 2026-05-05 - Anwenderdokumentationsindex fuer v2.4.0 angelegt"
---

# User Documentation

Diese Seite buendelt die Anwenderdokumentation fuer GitBook Worker.

## Start Here

- [GitBook Worker Anwenderanleitung v2.9.2](customer-installation.md) - zentrale Installation-, Betriebs- und Supportanleitung.
- [Configuration Reference](configuration-reference.md) - vollstaendige Referenz der unterstuetzten Konfigurationsschluessel.
- [Editorial Quality Tools](editorial-quality-tools.md) - Metrikreport und Abnahmedossier fuer redaktionelle Freigaben.
- [Editorial Quality Editor Review Report](reviews/editorial-quality-editor-review-report.md) - Vorlage fuer Kundenredakteure mit Pflichtenheftstatus und offenen Entscheidungen.
- [Editorial Quality Delivery Readiness Review](reviews/editorial-quality-delivery-readiness.md) - Vorlieferungsbewertung mit Bedingungen, Matrix und Nachlieferungsgrenzen.
- [Editorial Quality Delivery Evidence](reviews/editorial-quality-delivery-evidence.md) - konfigurierter Lieferlauf, Artefaktpaket und Dossierstatus.
- [Editorial Quality Requirements Matrix](reviews/editorial-quality-requirements-matrix.md) - Muss-, Soll-, Kann- und ERDA-Ergaenzungspunkte mit Nachweisen.
- [Editorial Quality Warning Groups](reviews/editorial-quality-warning-groups.md) - gruppierte Warnungen aus dem `release`-Lieferlauf.
- [Editorial Quality Fail Disclosure](reviews/editorial-quality-fail-disclosure.md) - offene Font-/Glyphen-Fails und Healing-Steps.
- [Editorial Quality Exit-Code Evidence](reviews/editorial-quality-exit-code-evidence.md) - reproduzierte Acceptance-Exit-Codes.
- [Editorial Quality Evidence Privacy](reviews/editorial-quality-evidence-privacy.md) - Datenschutzstufen fuer Evidenzauszuege.
- [Frequently Asked Questions](FAQs.md) - konkrete Fehlerbilder und schnelle Loesungen.
- [Release Notes](releases/README.md) - versionsbezogene Aenderungen und Upgrade-Hinweise.

## Topic Guides

- [Multilingual Content Guide](multilingual-content-guide.md) - mehrsprachige Inhaltsbaeume verwalten.
- [Contributor New Language](contributor-new-language.md) - neue Sprachbaeume anlegen.
- [Configure Document Types](Configure-Doc-Types.md) - dokumenttypbasierte Strukturierung.
- [Font Attribution](feature/font-attribution.md) - Font-Lizenzen und Attribution.
