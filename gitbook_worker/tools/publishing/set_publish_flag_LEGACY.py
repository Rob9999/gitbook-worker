...
New code should import from gitbook_worker.tools.utils.smart_manage_publish_flags instead.

This wrapper will be removed in a future version.

Legacy usage:
  python gitbook_worker/tools/publishing/set_publish_flag.py --commit <SHA> [--base <BASE_SHA>] [--reset-others] [--dry-run]

Typical GitHub Actions calls:
  # Push: compare before and after
  python gitbook_worker/tools/publishing/set_publish_flag.py --commit "$GITHUB_SHA" --base "${{ github.event.before }}" --reset-others

  # PR: compare base and head
  python gitbook_worker/tools/publishing/set_publish_flag.py --commit "${{ github.event.pull_request.head.sha }}" --base "${{ github.event.pull_request.base.sha }}" --reset-others
"""
import argparse
import sys
import warnings
