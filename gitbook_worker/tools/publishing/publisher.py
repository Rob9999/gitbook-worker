...
def _build_font_header(
    *,
    ...
    if emoji_font:
        ...
        # Define \panEmojiFont as a fontspec face switch for the emoji font.
        # This overrides the no-op stub from deeptex.sty and ensures that
        # \panEmoji{…} inside the TOC (.toc file) correctly activates the
        # colour emoji font when the TOC page is typeset.
        #
        # HISTORY: Previously this was generated by latex-emoji.lua and
        # injected via doc.meta['header-includes'].  However, Pandoc keeps
        # -H content and metadata header-includes in separate namespaces –
        # the Lua filter's modification was silently discarded.  Moving the
        # definition here (into the -H header file) guarantees it actually
        # reaches the LaTeX preamble.
        lines.append(
            f"  \\setfontface\\panEmojiFont{{{emoji_font}}}[Renderer=HarfBuzz]"
        )
    ...
    # Robust \panEmoji wrapper – survives LaTeX's fragile-command expansion
    # when headings are written to .toc / .aux (DeclareRobustCommand keeps
    # the token intact instead of expanding it away).  Placed unconditionally
    # so that \panEmoji always exists; when the emoji font is absent,
    # \panEmojiFont falls back to the deeptex.sty no-op stub.
    lines.append("\\DeclareRobustCommand*{\\panEmoji}[1]{{\\panEmojiFont#1}}")
    ...
    # Note: \panEmojiFont is defined above via \setfontface (inside
    # \IfFontExistsTF), and \panEmoji is defined unconditionally as a
    # DeclareRobustCommand.  The latex-emoji.lua filter wraps emoji spans
    # with \panEmoji{…} but no longer needs to inject the font definition
    # itself (Pandoc's -H header path is the only reliable way to get
    # content into the preamble).
    return "\n".join(lines) + "\n"
