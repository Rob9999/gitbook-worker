...
cd .github
pytest -q
python -m gitbook_worker.tools.workflow_orchestrator --profile default --dry-run
```
...
```bash
docker build -f gitbook_worker/tools/docker/Dockerfile -t erda-workflow-tools .
```
...
* `__main__.py` exposes `python -m gitbook_worker.tools.workflow_orchestrator` as the CLI entry
  point.
...
* `sources.py` extracts "Quellen"/"Sources" sections from Markdown files into a
  ...
  `python -m gitbook_worker.tools.quality.sources --help` for usage information.
* `link_audit.py` validates external links, image references, heading reuse and
  ...
  flags (`python -m gitbook_worker.tools.quality.link_audit --help`).
