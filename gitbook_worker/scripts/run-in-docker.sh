...
case $COMMAND in
    test)
        docker run --rm \
            ...
            "$IMAGE_TAG" \
            bash -c "cd /workspace && python3 -m pytest gitbook_worker/tests -v --tb=short"
        ;;
    test-slow)
        docker run --rm \
            -v "${WORKDIR}:/workspace" \
            ...
            -e DOCKER_LOG_DIR=/docker-logs \
            "$IMAGE_TAG" \
            bash -c "cd /workspace && python3 -m pytest gitbook_worker/tests -v -m slow --tb=short"
        ;;
    orchestrator)
        docker run --rm \
            -v "${WORKDIR}:/workspace" \
            ...
            -e DOCKER_LOG_DIR=/docker-logs \
            "$IMAGE_TAG" \
            bash -c "cd /workspace && python3 -m gitbook_worker.tools.workflow_orchestrator --root /workspace --manifest publish.yml --profile $PROFILE"
        ;;
    shell)
        docker run --rm -it \
            -v "${WORKDIR}:/workspace" \
