...
- **`../../build-pdf.ps1`** → Forwards to `gitbook_worker/scripts/build-pdf.ps1`
- **`../../build-pdf.sh`** → Forwards to `gitbook_worker/scripts/build-pdf.sh`
...
Executing: python -m gitbook_worker.tools.workflow_orchestrator --root ... --profile local
...
**Solution:**
Ensure you're running from repository root or `gitbook_worker/scripts/` directory.
...
**Solution:**
```bash
chmod +x gitbook_worker/scripts/build-pdf.sh
./build-pdf.sh
```
